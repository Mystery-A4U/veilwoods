...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    text = (
        f"👋 *This is A4U CR-DL Bot!*\n\n"
        f"Your one-stop Crunchyroll anime downloader 🎌\n\n"
        f"━━━━━━━━━━━━━━━━━━━━\n"
        f"✨ *Features*\n"
        f"━━━━━━━━━━━━━━━━━━━━\n"
        f"📥 Download anime up to *720p* (Free)\n"
        f"🎬 Download in *1080p* (Premium only)\n"
        f"🖼️ *Custom thumbnail* on downloads (Premium)\n"
        f"📦 Files sent directly to your chat\n"
        f"🌐 All subtitle languages embedded\n\n"
        f"━━━━━━━━━━━━━━━━━━━━\n"
        f"👑 *Premium System*\n"
        f"━━━━━━━━━━━━━━━━━━━━\n"
        f"• Earn *3 points* per referral 🔗\n"
        f"• Spend *3 points* = *2 hours* of Premium ⏳\n"
        f"• Use `/redeem` to activate premium\n\n"
        f"━━━━━━━━━━━━━━━━━━━━\n"
        f"📋 *Commands*\n"
        f"━━━━━━━━━━━━━━━━━━━━\n"
        f"`/link <url> [quality]` — Download anime\n"
        f"`/profile` — Your points & premium status\n"
        f"`/referral` — Get your referral link\n"
        f"`/redeem` — Spend points for premium\n"
        f"`/redeemcode <code>` — Redeem a gift code\n\n"
        f"━━━━━━━━━━━━━━━━━━━━\n"
        f"Your status: *{premium_badge}* | Points: *{u['points']}* 🪙\n\n"
        f"_Cooked by @Mystery\\_143_ 👨‍🍳"
    )
...
async def redeemcode(update: Update, context: ContextTypes.DEFAULT_TYPE):
    user = update.effective_user
    if not context.args:
        await update.message.reply_text("❌ Usage: `/redeemcode <code>`", parse_mode="Markdown")
        return
    ...
    msg = f"✅ *Code redeemed successfully!*\n\n"
    if reward:
...
async def broadcast(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not context.args:
        await update.message.reply_text(
            ...
            "`/gencode <points> <premium_hours> <max_uses>` — Generate redeem code",
            parse_mode="Markdown"
        )
        return
...
async def link(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    args = context.args
    if not args:
        await update.message.reply_text("❌ Usage: `/link <url> [quality]`", parse_mode="Markdown")
        return
    ...
    if quality not in VALID_QUALITIES:
        await update.message.reply_text(
            f"❌ Invalid quality.\nChoose: {', '.join(f'`{q}`' for q in VALID_QUALITIES)}",
            parse_mode="Markdown"
        )
        return
    ...
    status = await update.message.reply_text(
        f"⏳ Starting download{premium_tag}...\n🎬 Quality: `{quality}`",
        parse_mode="Markdown"
    )
    ...
    try:
        ...
        async for raw in process.stdout:
            ...
            if len(output_lines) % 15 == 0:
                try:
                    await status.edit_text(f"⏳ `{line}`", parse_mode="Markdown")
                except Exception:
                    pass
        ...
        await status.edit_text("📦 Download complete! Sending file...", parse_mode="Markdown")
        ...
        if saved_path and os.path.exists(saved_path):
            ...
            caption = f"🍥 *A4U CR-DL Bot*\n📁 `{os.path.basename(saved_path)}`\n🎬 Quality: `{quality}`"
            if user_is_premium:
                caption += "\n👑 _Premium download_"
            caption += "\n\n_Cooked by @Mystery\\_143_ 👨‍🍳"

            with open(saved_path, "rb") as f:
                if user_is_premium:
                    # Premium: send as video with thumbnail if available
            ...
            await status.edit_text("✅ Download complete! File saved on server.")

    except FileNotFoundError:
        await status.edit_text("❌ `yt-dlp` not installed on server.")
